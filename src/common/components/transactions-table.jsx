                ...
                {includeDetails &&
                  <React.Fragment>
                    ...
                    <td className={cx({ 'dim': source === 'to' }, "from")}>
                      <AddressDNS txn={txn} address={address} type="from" trunc={truncate} txSet={txSet} />
                    </td>
                    <td className={cx(source, "icon")}></td>
                    <td className={cx({ 'dim': source === 'from' }, "to")}>
                      <AddressDNS txn={txn} address={address} type="to" trunc={truncate} />
                    </td>
...
const AddressDNS = ({ txn, address, type, trunc = 20, txSet }) => {
  if (type === 'from') {
    if (txn && txn.fromDns) {
      return (
        <div className="value tooltip">
          ...
          <Link to={`/account/${from(txn, null, address)}`}>{truncate(txn.fromDns, { length: trunc })}</Link>
        </div>);
    } else {
    ...
    if (txn && txn.toDns) {
      return (
        <div className="value tooltip">
          ...
          <Link to={`/account/${to(txn, null, address)}`}>{truncate(txn.toDns, { length: trunc })}</Link>
        </div>);
    } else {
