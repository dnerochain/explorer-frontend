...
export default class StakesTable extends React.Component {
  ...
  loadMoreStakes() {
    ...
    this.setStakesDns(this.props.stakes, this.state.curStakeLength + TRUNC)
  }
  loadLessStakes() {
  ...
  setStakesDns = async (addresses, limit = 25) => {
    const stakes = addresses.slice(0, limit).map((x) => {
      return x.source ? x.source : x.holder
    });
    const domainNames = await dns.getDomainNames(stakes);
    addresses.map((x) => { x.dns = x.holder ? domainNames[x.holder] : x.source ? domainNames[x.source] : null });
              ...
              return (
                <tr key={address}>
                  <td className="address">
                    <AddressDNS address={address} dns={record.dns} truncate={truncate} />
                  </td>
...
const AddressDNS = ({ address, dns, truncate }) => {
  if (dns) {
    return (
      <div className="value tooltip">
        ...
        <Link to={`/account/${address}`}>{_truncate(dns, { length: truncate })}</Link>
      </div>);
  }
