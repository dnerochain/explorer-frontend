  ...
  getAllStakes(map) {
    ...
    stakeService.getAllStake(types)
      .then(res => {
        ...
        this.setStakesDns(sortedStakesBySource, 'source', 'sortedStakesBySource');
        this.setStakesDns(sortedStakesByHolder, 'holder', 'sortedStakesByHolder');
      })
      .catch(err => {
        console.log(err);
      });
  }
  setStakesDns = async(stakes, addrKey, stateKey) => {
    const slicedStakes = stakes.slice(0, 21).map((x) => x[addrKey]);
    const domainNames = await dns.getDomainNames(slicedStakes);
    stakes.map((x) => { x.dns = x[addrKey] ? domainNames[x[addrKey]] : null });
    let state = {};
